...
type dir struct {
	...
	customInfo  map[string]string // output generated by infoScript
}
...
func newDir(path string) *dir {
	...
	customInfo, err := readCustomInfo(files, path)
	if err != nil {
		log.Printf("generating info column: %s", err)
	}
...
func readCustomInfo(files []*file, path string) (map[string]string, error) {
	if len(gOpts.infoScript) == 0 {
		return nil, nil
	}
	...
	cmd := exec.Command(gOpts.infoScript, names...)
	cmd.Dir = path
	out, err := cmd.Output()
	if err != nil {
		return nil, err
