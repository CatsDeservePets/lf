...
var (
	...
	reRulerSub  = regexp.MustCompile(`%[apmcsfithPd]|%\{[^}]+\}`)
...
// (e.g., cursor moves) aren't removed. This prevents mismatches between the
// two functions and avoids misalignment when rendering the UI.
func stripAnsi(s string) string {
	...
	for i := 0; i < slen; i++ {
		...
		if r == gEscapeCode && i+1 < slen && s[i+1] == '[' {
			// TODO: test for additional sequences and improve handling
			// of broken sequences
